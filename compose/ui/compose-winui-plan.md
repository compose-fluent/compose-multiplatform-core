...
## Remaining concrete implementation items
- [ ] Replace the current narrow `WinUISkikoRenderHost` adapter with a production
  WinUI scene host that owns `WinUISkiaLayer` creation, root-content
  installation, load/unload handling, resize propagation, render invalidation
  coalescing, draw submission, diagnostics, and deterministic disposal.
- [ ] Move `WinUIComposeView` rendering and interop synchronization into one
  frame transaction: run measure/layout, drain queued interop mutations, submit
  Compose drawing, forward accessibility updates, and then publish diagnostics
  in a stable order on the WinUI UI thread.
- [ ] Replace the temporary WinUI JVM compile-source bridge with normal source-set
  dependencies once the required shared Skiko scene/rendering sources can be
  reused without compiling conflicting generic actuals.
- [ ] Implement visible-output graphics actuals needed by the original MPP sample:
  Skia-backed path operations, image decoding/drawing, brush/effect behavior,
  layer save/restore, clipping, transforms, and draw-bounds diagnostics.
- [ ] Implement the WinUI text editing bridge behind
  `WinUIPlatformTextInputService`: focus entry, native edit context/session
  ownership, composing text, committed text, selection updates, state sync from
  Compose to native IME, edit-menu commands, and cleanup on session replacement
  or owner disposal.
- [ ] Wire WinUI keyboard handling into text input and focus traversal so focused
  Compose text fields receive key/editing events, hosted WinUI controls keep
  native keyboard ownership, and Tab / Shift+Tab traversal remains predictable
  across Compose and `WinUIView` children.
- [ ] Expand the WinUI accessibility bridge from Skiko provider snapshots into
  full UI Automation parity for the Compose semantics tree: focus changes,
  bounds updates, click/custom actions, scroll actions, value changes,
  live-region-like notifications, hidden-node filtering, and native interop
  accessibility inclusion/exclusion.
- [ ] Route all WinUIView insertion, removal, z-order, layout, clipping, hit-test,
  and native property updates through the render-synchronized interop
  transaction queue, and remove ad hoc root-content synchronization callbacks
  once the transaction path covers the same behavior.
- [ ] Define the original MPP sample entrypoint for WinUI, add its WinUI JVM
  source set and `runWinUIMppSample` task, stage Windows App SDK/runtime
  resources, and keep AWT/Desktop-only APIs behind source-set guards.
- [ ] Add MPP sample resource validation for bundled images, fonts, strings,
  default language, PRI/resource staging, unpackaged runtime assets, and
  classpath guards that require `skiko-winui` while rejecting Skiko AWT/Desktop
  runtime artifacts.
- [ ] Keep the WinRT projection surface explicit by declaring only required
  `type(...)` entries for new WinUI, Windows SDK, text input, UI Automation,
  resource, and interop APIs; file `KWINRT-###` issues for generator/runtime
  failures instead of excluding legitimate WinMD dependency-chain types.
- [ ] Add focused validation tasks for each remaining area: render-output smoke,
  text-input/keyboard smoke, accessibility/UIA smoke, interop transaction smoke,
  MPP sample compile, MPP sample launch/window, MPP sample resources,
  MPP sample shutdown, and existing Android/Desktop/iOS regression retests.
