...
- **Latest exact-graph sweep:** `KWINRT-001`, `KWINRT-003`, `KWINRT-004`,
  `KWINRT-005`, `KWINRT-011`, `KWINRT-014`, `KWINRT-019`, and `KWINRT-022`
  were rechecked against the current compose-winui graph on `56c9267c`; see
  each issue's retest note for whether the evidence is runtime sample coverage,
  JVM test results, generated bytecode shape, or an explicit "not currently
  projected" finding. `KWINRT-021` is now classified as expected WinUI
  protected API behavior, not an upstream kotlin-winrt bug.
...
- **Latest retest note:** On `56c9267c`, `runWinUIViewSample` reaches
  `text input session cancellation` and exits successfully while using
  generated WinRT event accessors. The generated
  `kotlin-winrt/event-sources.tsv` for `:compose:ui:ui` contains 272 entries,
  and the current source tree has no `KWINRT-001` workaround references.
...
- **Status:** Not reproduced in the current compose-winui smoke path, but the
  generated API still needs a nullable-contract cleanup.
...
- **Resolution target:** Generate nullable Kotlin property types, or otherwise
  ...
  unset.
- **Latest retest note:** On `56c9267c`, the sample repeatedly reads
  `ContentControl.content` after WinUIView content removal checks such as
  `rootHost.content !is Canvas`, and `runWinUIViewSample` completes. `javap`
  still shows `ContentControl.getContent(): Object` / `setContent(Object)`,
  so this is not an active crash in the sample but still lacks a precise
  nullable Kotlin contract.
...
- **Latest artifact/runtime check:** On `56c9267c`, `javap` still shows
  `Window.setSystemBackdrop(SystemBackdrop)` with a non-null parameter shape,
  so the generated setter cannot be used to clear with null. The sample still
  validates `WindowBackdrop.Mica -> DesktopAcrylic -> None`; clearing uses the
  compose workaround and `runWinUIViewSample` passes.
...
- **Latest retest note:** On `56c9267c`, `:compose:ui:ui:compileKotlinWinuiJvm`
  still compiles `WinUIUriHandler.winui.kt` with generated
  `Launcher.launchUriAsync(uri, options, inputData)`, and the
  `WinUIUriHandlerTest` XML report shows 4 tests / 0 failures. This does not
  prove the one-argument overload is fixed.
...
- **Latest retest note:** On `56c9267c`, the sample completes with current
  compose mitigations: `Button`, `TextBox`, and `ToggleSwitch` are declared in
  `:compose:ui:ui`, clipboard ABI adapters are removed, and window activation
  no longer fails. This issue remains relevant for exact graph hygiene because
  duplicate generated support FQNs still exist; `KWINRT-020` now has concrete
  evidence for the interface-registry variant.
...
- **Latest artifact check:** On `56c9267c`, the current `:compose:ui:ui`
  WinUI projection output does not include an `AutomationProperties` class, and
  native accessibility participation remains deferred in compose-winui. There
  is no current runtime reproduction for this issue in the sample.
...
- **Latest retest note:** On `56c9267c`, `WinUIPlatformFocusOwnerTest` reports
  4 tests / 0 failures and `runWinUIViewSample` prints `owner focus request
  accepted`. The live native-control focus-transfer smoke is still not enabled,
  so this remains compose-side timing work rather than a current kotlin-winrt
  ABI blocker.
...
- **Status:** Closed as a kotlin-winrt bug. This is expected WinUI API shape:
  `UIElement.ProtectedCursor` is protected, so direct access requires a
  subclass/derived projected type rather than a public setter.
- **Observed in:** `Microsoft.UI.Xaml.UIElement.ProtectedCursor`
- **Symptom:** Compose cannot set the cursor through a public projected
  `UIElement` member because WinUI intentionally exposes cursor selection
  through the protected `UIElement.ProtectedCursor` property.
...
- **Latest retest note:** On `56c9267c`, `javap` shows
  `VirtualKeyModifiers` as a value class with `hasFlag`, `or`, `and`, and
  `constructor-impl` support. `WinUIPointerKeyboardModifiersTest` reports
  3 tests / 0 failures, covering individual flags, combined flags, and unknown
  bits.
