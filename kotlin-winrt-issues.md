...
- **Latest exact-graph sweep:** `KWINRT-001`, `KWINRT-003`, `KWINRT-004`,
  `KWINRT-005`, `KWINRT-011`, `KWINRT-014`, `KWINRT-019`, `KWINRT-021`, and
  `KWINRT-022` were rechecked against the current compose-winui graph on
  `56c9267c`; see each issue's retest note for whether the evidence is runtime
  sample coverage, JVM test results, generated bytecode shape, or an explicit
  "not currently projected" finding.
...
- **Latest artifact check:** On `56c9267c`, `javap` still shows
  ...
  protected members. Compose still uses the `IUIElementProtected`
  `ProtectedCursor` setter slot directly.
